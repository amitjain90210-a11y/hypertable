  ...
  while True:
    ...
    for cell in cells:
      ...
      if cell.key.column_family == "StartRow":
        cur['StartRow'] = cell.value.decode('latin-1') if isinstance(cell.value, bytes) else cell.value
      elif cell.key.column_family == "Location":
        cur['Location'] = cell.value.decode('latin-1') if isinstance(cell.value, bytes) else cell.value
      else:
