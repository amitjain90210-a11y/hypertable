...
void IndexTables::add(const Key &key, uint8_t flag,
  ...
  if (value_index_mutator) {
    ...
    p = tmp.append_to(p);
    *p++  = ',';
    if (escaped_value_len) memcpy(p, escaped_value, escaped_value_len);
    p     += escaped_value_len;
    *p++  = '\t';
    if (escaped_qualifier_len) memcpy(p, escaped_qualifier, escaped_qualifier_len);
    p     += escaped_qualifier_len;
    *p++  = '\t';
    if (escaped_row_len) memcpy(p, escaped_row, escaped_row_len);
    p     += escaped_row_len;
    *p++  = '\0';
    k.row = sb.base;
