...
void OperationRecoverRanges::create_futures() {
  ...
  future = std::make_shared<RecoveryStepFuture>("replay", m_plan_generation);
  m_context->recovery_state().install_replay_future(id(), future);

  // Install "prepare" future
  future = std::make_shared<RecoveryStepFuture>("prepare", m_plan_generation);
  m_context->recovery_state().install_prepare_future(id(), future);

  // Install "commit" future
  future = std::make_shared<RecoveryStepFuture>("commit", m_plan_generation);
